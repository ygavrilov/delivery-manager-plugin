...
When implementing a Vue.js feature:
...
5. Every API call in the store must have: loading state, error handling, and success feedback

Principles:
- **KISS**: simplest structure that works — no abstractions without proven reuse
- **DRY**: extract to a composable only when stateless logic is used in 2+ places
...
- **Convention over invention**: follow existing project file and naming patterns
