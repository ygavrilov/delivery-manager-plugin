---
name: core-skill-structure
category: core
type: rule
description: Defines how skills are structured, authored, and referenced in this plugin. Use when creating or editing a skill, or checking skill-authoring conventions.
worker-hint: senior
---

Meta-skill for skill authoring. Covers four skill categories (core, lang, framework, project), two skill types (workflow, rule), file structure (SKILL.md + reference.md + optional extras), frontmatter schema, and body conventions.

Load when creating or updating any skill.

## Additional Resources

- For full conventions, frontmatter schema, and examples, see [reference.md](reference.md)
